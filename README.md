...
## 1. 프로젝트 개요
본 서비스는 유저 계정의 해시태그를 기반으로 복수의 SNS에 게시된 게시물 중 유저의 해시태그가 포함된 게시물들을 하나의 서비스에서 확인할 수 있는 통합 Feed 어플리케이션 입니다.
이를 통해 본 서비스의 고객은 하나의 채널로 유저 또는 브랜드의 SNS 노출 게시물 및 통계를 확인할 수 있습니다.

### ✨ 사용자 주요 기능
- 이메일로 발송된 인증코드 비교를 통한 회원가입, 로그인
- 게시물 조회 및 좋아요, 공유 기능
- 특정 해시태그 일자별, 시간별 검색 필터링
